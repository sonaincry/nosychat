...
const emptyProfile = {
  ...
  voiceChatAvailable: false, bio: '', photoUrls: [] as string[], preferredPlayTime: '', modeIds: [] as string[],
};
...
export default function PartyMatch({ user, hubConnection }: Props) {
  ...
  const openProfile = (gameId = swipeGameId || games[0]?.id || '') => {
    ...
    setProfile(saved ? {
      ...
      modeIds: saved.modes.map(mode => mode.id),
    } : { ...emptyProfile, gameId });
    setShowProfile(true);
  };
  ...
  return <div className="party-shell"><div className="party-window">
    ...
    <main className="party-main">
      ...
      <section className="party-hero party-swipe-hero">
        ...
        <div className="party-hero-actions"><button className="party-secondary" onClick={() => openProfile()}><ShieldCheck size={18} /> Hồ sơ game</button></div>
      </section>
      ...
      <section className="swipe-workspace">
        ...
        <div className="swipe-deck">
          {loadingCandidates ? <div className="party-empty">Đang tìm người chơi…</div> : !candidate ? <div className="party-empty"><Gamepad2 size={40} /><h3>Đã xem hết hồ sơ phù hợp</h3><p>Cập nhật hồ sơ hoặc thử game và số người khác.</p></div> : <article className="swipe-card">
            ...
            <div className="swipe-card-copy"><h2>{candidate.displayName}</h2><div className="party-tags">{candidate.serverRegion && <span>{candidate.serverRegion}</span>}<span>{candidate.language}</span>{candidate.rank && <span>{candidate.rank}</span>}{candidate.preferredRole && <span>{candidate.preferredRole}</span>}{candidate.modes.map(mode => <span key={mode.id}>{mode.name}</span>)}</div>{candidate.bio && <p>{candidate.bio}</p>}<div className="swipe-meta">{candidate.voiceChatAvailable && <span><Headphones size={15} /> Có voice chat</span>}{candidate.preferredPlayTime && <span><Clock3 size={15} /> {candidate.preferredPlayTime}</span>}</div></div>
      ...
      {myParties.length === 0 ? <div className="party-empty"><Users size={38} /><h3>Chưa có party</h3><p>Mutual match đầu tiên sẽ tự động tạo Party và ChatGroup.</p></div> : <div className="party-grid">{myParties.map(party => <article className="party-card" key={party.id}>
        ...
        <div className="party-tags">{party.serverRegion && <span>{party.serverRegion}</span>}<span>{party.language}</span>{party.rank && <span>{party.rank}</span>}</div>
